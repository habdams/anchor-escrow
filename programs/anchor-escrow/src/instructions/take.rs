...
impl<'info> Take<'info> {
    // Transfer token from the taker to maker
    pub fn deposit(&mut self) -> Result<()> {
        ...
        let cpi_ctx = CpiContext::new(self.token_progam.key(), cpi_accounts);
    ...
    pub fn withdraw_and_close_vault(&mut self) -> Result<()> {
        ...
        let cpi_ctx =
            CpiContext::new_with_signer(self.token_progam.key(), cpi_accounts, signer_seeds);
